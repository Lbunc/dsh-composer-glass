 ...
 *
 * WHAT IT DOES
 * Turns the composer card — plus the two floating surfaces that share its
 * screen region, the todo dock above it and the back-to-bottom button — into
 * one uniformly translucent material. It is a MATERIAL, not a refractor:
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * reverts, check these first: `[data-composer-card]` and
     * `[data-testid="todo-panel"]` are stable data attributes and are preferred
     * wherever one is available — only the back-to-bottom button has none.
     */
     ...
     * effects. The tint is a variable so it can differ per theme without
     * duplicating the recipe.
     */
    const GLASS_SURFACE = [
      '  background-color: color-mix(in srgb, var(--dsh-glass-tint) calc(' + MATERIAL.tint + ' * 100%), transparent) !important;',
    ...
    const CSS = [
      ...
      ':root {',
      '  --dsh-glass-tint: var(--dsw-specific-input-major);',
      ...
      // One material, three surfaces. The composer card keeps its own 22px
      // radius; the todo dock and the back-to-bottom button keep whatever
      // shape their shipped rules give them, so only the material is shared.
      ...
      // No sheen here: 34px is too small for the gradient to read.
      ...
      '  background-color: color-mix(in srgb, var(--dsh-glass-tint) calc(' + MATERIAL.tintHover + ' * 100%), transparent) !important;',
      '}',
