...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // The primitives package is in the shell's static seed table (like react), so
    // it needs no `dsh.client.external` edge and arrives with the seed.
    ...
    const MESSAGES = {
      zh: {
        'row.title': '输入区毛玻璃',
        'row.desc': '让输入框及其周边（任务清单、回到底部、底部状态栏）统一为半透明毛玻璃材质',
        'opt.on': '开启',
        'opt.off': '关闭'
      },
      en: {
        'row.title': 'Composer-area frosted glass',
        'row.desc': 'Turn the composer and its surrounding surfaces (todo dock, back-to-bottom, status bar) into one translucent frosted-glass material',
        'opt.on': 'On',
        'opt.off': 'Off'
      }
    }
    ...
    const CSS = [
      ...
      // No sheen here: 34px is too small for the gradient to read. ONLY the
      // outer drop shadow differs from the shared material: redefining the halo
      // variable on the button leaves its box-shadow list — ring, top
      // highlight, bottom shade — byte-identical to every other surface.
      ...
      '  height: 36px; font: inherit; font-size: 14px; line-height: 22px;',
      '  color: var(--dsw-alias-label-primary, #e8e8ea);',
      '  cursor: pointer; border: none; border-radius: 18px;',
      '  align-items: center; gap: 12px; padding: 0 14px; display: inline-flex;',
      '}',
      '.' + PREFIX + '-selector:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.22)); }',
      '.' + PREFIX + '-chevron { flex: none; }'
    ].join('\n')
    ...
    function GlassRow(props) {
      ...
      const options = [
        { id: 'on', label: t('opt.on') },
        { id: 'off', label: t('opt.off') }
      ]
      ...
      return React.createElement(
        ...
        // The same selector pill + Menu primitive the shipped Language row uses
        // (`dsh-client-locale`), so this row cannot drift from its neighbours.
        React.createElement(primitives.Menu, {
          open,
          onClose: () => setOpen(false),
          items: options,
          selectedId: on ? 'on' : 'off',
          onSelect: (id) => {
            setEnabled(id === 'on', true)
            setOpen(false)
          },
          align: 'end',
          portal: true,
          anchor: React.createElement(
            ...
            t(on ? 'opt.on' : 'opt.off'),
            React.createElement(primitives.IconChevronDownOutline14, { className: PREFIX + '-chevron' })
          )
        })
      )
    }
